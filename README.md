...
v0.3.0 is the **Team-Ready Profiles** release. A profile becomes a portable, versioned environment blueprint that bundles an AI agent, its skills, MCP servers, and a structured prompt. Teams distribute profiles through vaults; a new developer installs one profile and is ready to code in 30 seconds. This release also adds native Claude Code agent profiles and multi-provider LLM support.

### Claude Code agent profiles & multi-provider LLM
...
- **Hexagonal provider boundaries** — agent providers (claude-code, opencode) and LLM providers are isolated; an architecture test enforces that concrete LLM adapters are only imported from the composition root
- **Multi-stage Docker** — slim runtime image (~50MB) for production, full CI image for the test matrix
- **Slim-build regression guard** — the binary still builds with `--no-default-features --features tui` only
