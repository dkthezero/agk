...
pub static TEMPLATES: LazyLock<Vec<ArchetypeTemplate>> = LazyLock::new(|| vec![
    ArchetypeTemplate {
        id: "code-reviewer".to_string(),
        name: "Code Reviewer".to_string(),
        description: "Senior code reviewer; direct & critical; triggers after code changes".to_string(),
        defaults: {
            ...
            m.insert("format".to_string(), "Concise bullets, max 5 items".to_string());
            ...
            m.insert("constraints".to_string(), "Always include a line reference; never approve without reading".to_string());
            m
        },
        default_tools: vec!["Read".to_string(), "Glob".to_string(), "Grep".to_string(), "LSP".to_string()],
        default_permission_mode: Some("default".to_string()),
    },
    ArchetypeTemplate {
        id: "feature-implementer".to_string(),
        name: "Feature Implementer".to_string(),
        description: "Senior engineer; pragmatic & thorough; triggers on implementation requests".to_string(),
        defaults: {
            let mut m = HashMap::new();
            m.insert("role".to_string(), "Senior engineer".to_string());
            m.insert("style".to_string(), "Pragmatic and thorough".to_string());
            m.insert("triggers".to_string(), "When user asks for implementation".to_string());
            ...
            m.insert("constraints".to_string(), "Always write tests; follow existing patterns".to_string());
            m
        },
        default_tools: vec!["Read".to_string(), "Glob".to_string(), "Grep".to_string(), "Bash".to_string(), "Write".to_string(), "Edit".to_string()],
        default_permission_mode: Some("default".to_string()),
    },
    ArchetypeTemplate {
        id: "security-auditor".to_string(),
        name: "Security Auditor".to_string(),
        description: "Security engineer; cautious & explicit; triggers on security keywords".to_string(),
        defaults: {
            let mut m = HashMap::new();
            m.insert("role".to_string(), "Security engineer".to_string());
            m.insert("style".to_string(), "Cautious and explicit".to_string());
            m.insert("triggers".to_string(), "When security keywords detected".to_string());
            m.insert("format".to_string(), "Risk + Mitigation + Verification".to_string());
            m.insert("boundaries".to_string(), "IN SCOPE:\nSecurity review, dependency audit, input validation\n\nOUT OF SCOPE:\nGeneral feature work".to_string());
            m.insert("constraints".to_string(), "Never dismiss a finding without evidence".to_string());
            m
        },
        default_tools: vec!["Read".to_string(), "Glob".to_string(), "Grep".to_string(), "Bash".to_string()],
        default_permission_mode: Some("default".to_string()),
    },
    ArchetypeTemplate {
        id: "documentation-writer".to_string(),
        name: "Documentation Writer".to_string(),
        description: "Technical writer; clear & structured; triggers after API changes".to_string(),
        defaults: {
            let mut m = HashMap::new();
            m.insert("role".to_string(), "Technical writer".to_string());
            m.insert("style".to_string(), "Clear and structured".to_string());
            m.insert("triggers".to_string(), "After public API changes".to_string());
            m.insert("format".to_string(), "Reference docs with examples".to_string());
            m.insert("boundaries".to_string(), "IN SCOPE:\nAPI docs, README updates, changelog entries\n\nOUT OF SCOPE:\nMarketing copy".to_string());
            m.insert("constraints".to_string(), "Every public item must have an example".to_string());
            m
        },
        default_tools: vec!["Read".to_string(), "Glob".to_string(), "Grep".to_string(), "Write".to_string(), "Edit".to_string()],
        default_permission_mode: Some("default".to_string()),
    },
    ArchetypeTemplate {
        ...
        defaults: {
            ...
            m.insert("triggers".to_string(), "When source files lack tests".to_string());
            m.insert("format".to_string(), "Unit → Integration → Edge cases".to_string());
            m.insert("boundaries".to_string(), "IN SCOPE:\nTest generation, coverage analysis, fixture creation\n\nOUT OF SCOPE:\nProduction bug fixes".to_string());
            m.insert("constraints".to_string(), "100% branch coverage for new code".to_string());
            m
        },
        default_tools: vec!["Read".to_string(), "Glob".to_string(), "Grep".to_string(), "Bash".to_string(), "Write".to_string()],
        default_permission_mode: Some("default".to_string()),
    },
    ArchetypeTemplate {
        id: "custom".to_string(),
