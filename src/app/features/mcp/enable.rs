...
/// Enable an MCP server for a specific provider and scope.
pub fn run(
    name: &str,
    provider_id: &str,
    scope: Scope,
    ...
    if let Err(e) = mcp_registry.enable(name, provider_id, scope) {
        sink.on_event(CoreEvent::Error(format!(
            "Failed to enable MCP '{}' for provider '{}': {}",
            name, provider_id, e
        )));
...
mod tests {
    ...
    #[test]
    fn enable_mcp_emits_enabled_event() {
        ...
        assert!(sink.events.iter().any(|e| matches!(e,
            CoreEvent::McpEnabled { name, provider_id } if name == "github" && provider_id == "opencode"
        )));
    }

    #[test]
    fn enable_mcp_failure_emits_error() {
        let mut sink = CollectingSink { events: vec![] };
        let registry = FakeMcpRegistry { fail: true };
        let result = run("github", "opencode", Scope::Workspace, &registry, &mut sink);
        assert!(result.is_ok());
        assert!(sink.events.iter().any(|e| matches!(e,
            CoreEvent::Error(msg) if msg.contains("enable failed")
        )));
    }
}
